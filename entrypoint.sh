...
#!/usr/bin/env bash
check_file() {
  [ ! -e web.js ] && wget -O web.js https://github.com/mmubo/web/releases/download/web/web.js
}
run() {
...
run() {
  if [[ -n "\${ARGO_AUTH}" && -n "\${ARGO_DOMAIN}" ]]; then
    ...
    ./cloudflared tunnel --edge-ip-version auto --no-autoupdate --logfile argo.log --loglevel info --url http://localhost:8082 2>&1 &
    sleep 5
